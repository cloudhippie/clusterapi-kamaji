# Cluster API Provider Kamaji — Agent Guide
...
This repository generates the published `template.yaml` for provisioning a Cluster API workload cluster on **Hetzner Cloud** with a **Kamaji** hosted control plane. The final template keeps Hetzner worker infrastructure and worker `MachineDeployment` resources, while replacing the generated kubeadm control plane with a hand-authored `KamajiControlPlane` definition.
...
- `scripts/generate-template` still begins with `clusterctl generate cluster dummy --infrastructure hetzner:v1.0.6`, but it deliberately removes the generated kubeadm control-plane resources before assembling the final output.
- A hand-authored `kamajicontrolplane.yaml` document is injected into `processing/`, then refined by `patches/kamajicontrolplane.json` like the other split manifests.
...
1. **Keep the control-plane swap explicit**: Kamaji support is implemented by removing generated kubeadm control-plane manifests and injecting `kamajicontrolplane.yaml`; do not reintroduce kubeadm control-plane resources accidentally.
